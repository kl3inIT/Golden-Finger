            ...
            <div class="flex flex-wrap justify-between items-center mx-auto min-[1600px]:max-w-[1600px] min-[1400px]:max-w-[1320px] min-[1200px]:max-w-[1140px] min-[992px]:max-w-[960px] min-[768px]:max-w-[720px] min-[576px]:max-w-[540px]">
                <div class="section-title-2 w-full mb-[20px] pb-[20px] flex flex-col justify-center items-center">
                    ...
                    <p class="max-w-[400px] mt-[15px] text-[14px] text-[#777] text-center leading-[23px]">
                        <c:choose>
                            <c:when test="${param.step == '2'}">Create your new password</c:when>
                            <c:otherwise>Enter your email to verify your account</c:otherwise>
                        </c:choose>
                    </p>
                </div>
                            ...
                            <div class="gi-login-container p-[30px] max-[575px]:p-[15px] border-[1px] border-solid border-[#eee] rounded-[5px] text-left bg-[#fff]">
                                <div class="gi-login-form">
                                    ...
                                    <c:if test="${param.step != '2'}">
                                        <form action="forgot-password" method="post" class="flex flex-col" id="emailVerificationForm">
                                            <div class="flex flex-col">
                                                <span class="gi-login-wrap flex flex-col w-full">
                                                    <label class="mb-[10px] text-[#4b5966] text-[15px] font-medium tracking-[0] leading-[1]">Email*</label>
                                                    <input type="email" name="email" id="email" value="${sessionScope.email}" placeholder="Enter your email" class="w-full mb-[27px] px-[15px] bg-transparent border-[1px] border-solid border-[#eee] rounded-[5px] text-[#777] text-[14px] outline-[0] h-[50px]" required>
                                                </span>
                                        ...
                                        <form action="forgot-password" method="post" class="flex flex-col" id="resetPasswordForm">
                                            <div class="flex flex-col">
                                                ...
                                                <span class="gi-login-wrap flex flex-col w-full">
                                                    ...
                                                    <input type="password" name="newPassword" id="newPassword" placeholder="Enter new password" class="w-full mb-[27px] px-[15px] bg-transparent border-[1px] border-solid border-[#eee] rounded-[5px] text-[#777] text-[14px] outline-[0] h-[50px]" required>
                                                </span>

                                                <span class="gi-login-wrap flex flex-col w-full">
                                                    <label class="mb-[10px] text-[#4b5966] text-[15px] font-medium tracking-[0] leading-[1]">Confirm Password*</label>
                                                    <input type="password" name="confirmPassword" id="confirmPassword" placeholder="Confirm new password" class="w-full mb-[27px] px-[15px] bg-transparent border-[1px] border-solid border-[#eee] rounded-[5px] text-[#777] text-[14px] outline-[0] h-[50px]" required>
                                                </span>
                                                ...
                                                <span class="gi-login-wrap gi-login-btn mt-[30px] flex flex-row justify-between items-center">
                                                    <span class="text-[#777] text-[14px]">
                                                        <a href="forgot-password" class="text-[#4b5966] hover:text-[#5caf90]">Back to Email Verification</a>
                                                    </span>
                                    ...
                                    <c:if test="${not empty requestScope.success}">
                                        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mt-4" role="alert">
                                            <span class="block sm:inline mb-2">${requestScope.success}</span>
                                            <div class="mt-3">
                                                <a href="login" class="text-[#4b5966] hover:text-[#5caf90] font-medium">Back to Login</a>
                                            </div>
                                        </div>
                                    </c:if>
                                </div>
        ...
        <script>
            ...
            if (confirmPasswordInput) {
                confirmPasswordInput.addEventListener('input', function () {
                    ...
                    if (password !== confirmPassword) {
                        this.setCustomValidity('Passwords do not match ConfirmPassword');
                    } else {
            ...
            if (resetPasswordForm) {
                resetPasswordForm.addEventListener('submit', function (e) {
                    ...
                    if (password !== confirmPassword) {
                        e.preventDefault();
                        alert('Passwords do not match c');
                    }
